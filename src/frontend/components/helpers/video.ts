...
export function getFirstOutputIdWithAudableBackground(outputIds: string[] = [], _updater: any = null) {
    if (!outputIds.length) outputIds = getAllNormalOutputs().map((output) => output.id)

    return (
        outputIds.find((id) => {
            const output = get(outputs)[id]
            if (!output || output.stageOutput) return false
            if (get(audioChannelsData)[id]?.isMuted) return false

            const style = get(styles)[output.style || ""]
            const layers = Array.isArray(style?.layers) ? style.layers : ["background"]
