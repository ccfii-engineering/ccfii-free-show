...
export async function getUpdateData(currentVersion: string, includeBeta: boolean): Promise<UpdateData> {
    const response = await fetch("https://api.github.com/repos/ccfii-engineering/ccfii-free-show/releases")
    const data = await response.json()
    const latestAll = data.filter((release: any) => release.draft === false)[0]
    const latestRelease = data.filter((release: any) => release.draft === false && release.prerelease === false)[0]
    ...
    return { latestVersion, changelog, hasUpdate: !!latestVersion && currentVersion !== latestVersion }
}
